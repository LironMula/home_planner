...
- Alt-4 basement-to-ground and ground-to-living stairs are L-shaped. The living-to-top stair is a separate, adjacent U-shaped stair with unequal runs, confirmed by A14/H treads and the top-floor A13 arrival marker. The earlier instruction to stack it over the lower stair was withdrawn. Do not reuse the incoming lower-stair AREA 1575 as the outgoing footprint. See the source handles and coordinates in `docs/alt4-v2-layer-audit.json` under `verticalCore`.
- The future elevator is the repeated 1.00 x 1.50 m H-layer contour (11DF, 22AD, 1D5D, 1546), not an AREA stair contour. Keep the same shaft opening through ground, living, and top floors and the corresponding ceilings below; retain the basement base and top roof. Top-floor A17 1568/1564/1538 encloses this reservation. H 1251/1252 is the red-X projection where the lower stair cannot continue, not another flight or a slab opening over its whole rectangle.
- Fit the Alt-4 roof as one circular arc over the enclosed A17 wall contour with a 10 cm drip-edge overhang, never over the open terraces. Use the section's 586 cm structural radius; across the current enclosed wall span the finished arc rises about 2.05 m at the wall edges. The top story has no automatic flat ceiling slab beneath this roof.
...
- Pantry storage uses the two complete 4.15 m opposing cabinet runs. Keep both 0.60 m deep banks inside the pantry wall faces, with about 1.07 m of clear aisle between them; orient both fronts toward the pantry and render one visible handle per door panel. The north run is a 2.50 m kitchen tall-storage assembly: preserve its built-in 1.10 m refrigerator bay and the storage bays around it instead of flattening it to a generic closet.
...
- Reconstruct the master walk-in closet as two opposing 3.25 m ceiling-height open wardrobe banks. The DWG shows 0.45 m and 0.60 m depths, but the user approved a 0.50 m depth for both banks. Hold their outer backing faces fixed, yielding a 0.95 m clear aisle. The centered light-cream 1.00 m opening is the master-suite entry and must lead directly into that aisle.
- Register the master walk-in to its dedicated entrance wall, not to a nearby exploded wardrobe trace: both closet-bank fronts align at the same 2D datum and the centered door must lead directly into the 0.90 m aisle. Do not add a second master-bedroom door beside the walk-in.
...
- Reconstruct the walk-in entrance wall as a continuous host before cutting its centered 1.00 m door opening; do not attach the door to a short neighboring CAD fragment. Never infer a free-standing suite divider from exploded wardrobe fronts or swing arcs.
...
- Check each stair's source footprint, lower endpoint, outgoing-triangle upper endpoint, and destination-floor landing after rotation. For the Alt-4 L-shaped flights, use the marked podest/landing and triangle endpoint as registration anchors; do not force their full bounding boxes to overlap because the entrance flight occupies the right-hand leg of the core.
