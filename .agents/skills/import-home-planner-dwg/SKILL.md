...
5. Infer symbols as clusters. Combine enclosure, dimensions, nearby fixtures, wall contact, door access, repeated symbols, and room function. Color or layer is supporting evidence, not proof. Decode stair direction from the complete run and its arrival marker. Distinguish incoming, outgoing, and crossed-out projected flights on each sheet. Successive stairs may occupy different footprints and use different turn shapes; verify each against both its departure and destination rather than forcing vertical stacking.
...
7. Preserve intentional overlap only: sink in counter, mirror on wall, appliance in cabinet. Flag furniture intersections, fixtures crossing walls, detached openings, inaccessible doors, and faces or handles pointing outside the room. Check rotated footprints in world coordinates rather than their unrotated JSON boxes.
   Reject zero or negative object dimensions before and after the site transform. For wall-abutting fixtures, intersect the source wall at the fixture's station and use its adjacent face; a whole hatch's bounding edge may be on the opposite side of a stepped wall.
...
- Verify each stair footprint, run direction, upper end, and landing against the source. Classify an AREA contour before assigning it to an outgoing flight: it may describe an incoming stair or a projected lower core. Reconcile departure treads with arrival treads, markers, and walls, including verified supplementary layers. Cut the floor and ceiling opening from the union of the individual flights and landing; never use the enclosing stair bounding box. A floor slab is cut by the arriving stair and its ceiling by the departing stair. Elevator shafts and double-height voids are separate architectural openings with their own vertical extent, not unused parts of a stair bounding box.
